...
%%%%%%%%%%
\vskip2mm
\red{Should the last two judgments above mention ``valid'' in their conclusions?}

\red{Also, validity is never referred to elsewhere.  Is there a
  blanket assumption that all contexts are valid?}
%%%%%%%%%%    
    \caption{Contexts}
...
\begin{figure*}[t]
    ...
    \end{prooftree}
    \red{rec is never defined.  Is this something that's left over from an old version of the document?}
    ...
    \end{prooftree}
    \red{Why $K = \texttt{(type)}$ rather than putting $A::\texttt{(type)}$ after the $\vdash$?}
...
%%%%%%%%%%
\vskip2mm
\red{In the last rule, I think the idea is that we have to look up
$bt$ and $tcn$ in Figure 13. I'm a little confused here since in Figure 1
$bt$ is one form of $tcn$. Is there a typo here?}
%%%%%%%%%%    
    \caption{Contexts}
...
\red{``Normal type'' is never defined. Is this what's called a Type Value in the grammar?}    
