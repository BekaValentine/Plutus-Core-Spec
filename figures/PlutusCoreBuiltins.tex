...
þ       \texttt{greaterThanEqualsInteger}   &  \forall s :: size.\ integer_s \to integer_s \to boolean  &  s!i_0 , s!i_1   &   i_0 >= i_1\\
        ...
        \texttt{resizeInteger}   &  \forall s_0, s_1 :: size.\ size_{s_1} \to integer_{s_0} \to integer_{s_1}  &   z, s_1!i   & s_1!i & -2^{8s_1-1} \leq i < 2^{8s_1-1}\\
        %&&\\
        
        \texttt{intToByteString}  &   \forall s_0, s_1 :: size.\ size_{s_1} \to integer_{s_0} \to bytestring_{s_1}  &  z, s_1!i           & \text{the binary representation of $i$} &-2^{8s_1-1} \leq i < 2^{8s_1-1}\\
        ...
        \texttt{verifySignature}   &  \forall s_0, s_1, s_2 :: size.\ bytestring_{s_0} \to bytestring_{s_1} \to bytestring_{s_2} \to boolean  &   key, dat, sig           & verifySignature \  key \  dat \  sig\\
        %&&\\
        ...
        \texttt{txhash}   &   bytestring_{\conT{256}}   &   \textrm{---}  & txh\\
        %&&\\
        
        \texttt{blocknum}  &  \forall s :: size.\ size_s \to integer_s  & z & bnum & 0 \leq bnum < 2^{8s-1}\\
        %&&\\
        
        \texttt{blocktime} &  \mathit{datetime}  &  \textit{---}  & btime\\
    \end{array}\)
