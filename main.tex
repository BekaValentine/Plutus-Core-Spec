...
%Finally, type synthesis for built-in operations (\(\conSig{n}{\repetition{S}}{T}\)) is given in tabular form rather than in inference rule form, in Figure \ref{fig:Plutus_core_builtins}, which also gives the reduction semantics. The types in the arguments column constitute $\repetition{T}$, and the type in the return column constitutes $T'$, in the judgment form. The same is done for synthesis of built-in computations (\(\compbuiltinSig{n}{T}\)).
