...
compile time.%
%%%
\redfootnote{Is this true?  Are we actually doing this at the moment or does it need extra analysis?}
%%%
 In contrast, IELE uses unbounded integers, which never
overflow, but require that the gas used is calculated at run time. We
use \Fomega{} (as opposed to F)%
%%%
\redfootnote{F$^{\omega}$ or F$_{\omega}$?}
%%%
because it supports types indexed by
...
the presence of type left %
%%%
\red{[Eh?]} 
%%%
fixed points, it's possible to give a valid
...
declaration to do. Precisely which combinators are defined is up to
...
%%%%%%%%%%
\red{It might be helpful to have the figures containing the syntax, type system, etc.
closer to the point where they're talked about.  Almost all of the text appears on
page 1, then we get 8 or 9 pages of formulas.}  
\red{Indeed, why not have (sub)sections rather than figures?  I think this
would certainly be sensible for things like the progress and preservation proofs.}
%%%%%%%%%%

\section{Syntax}

The grammar of Plutus Core is given in Figures \ref{fig:Plutus_core_grammar} and 
...
%%%
\redfootnote{``Binary'' made me think of 0101011010.  ``Single-argument form''?}
%%%
To the extent that
...
%%%%%%%
\red{Some more explanation of the syntax would be helpful here. I
  found it quite difficult to work out what the purpose of some of the 
  syntax was.  I think that there are also some typos.  [I was working from
    a printed version which missed out the definition of $V$, but I've just noticed that this 
    has been corrected in this version. Some of my comments may be based on the old version.]
\begin{itemize}
\item Are Values also Terms?
%
\item Under Term (and maybe also under Value), should $\abs{\alpha}{K}{V}$ be $\abs{\alpha}{K}{M}$?
%
\item I was initially quite confused by there being two \texttt{lam}s and an \texttt{abs}, 
all types of abstraction.  Some up-front explanation (maybe showing how they correspond
to more standard things like $\lambda x:S.M$ and $\lambda \alpha::K.V$) might be helpful for the reader.
%
\item I found it difficult to understand the roles of ``Type'' and
  ``Type Value'' just looking at the syntax in isolation.  I think
  Type Values are Types which have been normalised according to Figure
  7.  It would be useful to explain this in advance, and also to say
  why type normalisation is useful.  Also, would something like ``Normalised Type'' be 
  more informative than ``Type Value''?
%
\item It might also be worth pointing out that the syntax of Type Values is essentially
the same as that of Types, except where application is concerned.
%
\item What's neutral about a ``neutral'' type?
\item An explanation of the \texttt{size} kind would be useful as well.
\item I don't think $L$, $U$, $C$ and $R$ are ever used.  That's unimportant though.
 \end{itemize}
[Sorry, I couldn't get this to appear closer to the actual grammar.]
}
%%%%%%%
...
In figure \ref{fig:Plutus_core_reduction}, we define a standard eager, small-step contextual semantics%
%%%
\redfootnote{The term ``contextual semantics'' wasn't familiar to me and Google wasn't much help.  
However the style of semantics \textit{is} familiar: I'm not sure what I'd call it though.
Is there a reference for this that might help the reader?} 
%%%
for Plutus Core in terms of the reduction relation for types (\(\typeStep{A}{A'}\)) and terms (\(\step{M}{M'}\)), which incorporates both $\beta$ reduction and contextual congruence. We make use of the transitive closure of these stepping relations via the usual Kleene star notation.

In the context of a blockchain system, it can be useful to also have a step indexed version of stepping, indicated by a superscript count of steps (\(\multistepIndexed{M}{n}{M'}\)). In order to prevent transaction validation from looping indefinitely, or from simply taking an inordinate amount of time, which would be a serious security%
%%%
\redfootnote{Is it really a \textit{security} flaw?}
%%%
 flaw in the blockchain system, we can use step indexing to put an upper bound on the number of computational steps that a program can have. In this setting, we would pick some upper bound $\mathit{max}$ and then perform steps of terms $M$ by computing which $M'$ is such that \(\multistepIndexed{M}{\mathit{max}}{M'}\).%
%%%
\redfootnote{This sentence is a bit convoluted.  Is it saying that we perform at most \textit{max}
reductions and then look at what we get?}
...
\section{Basic Validation Program Structure\protect\redfootnote{I'm not sure what this is supposed to mean...}}
...
\red{[I didn't get any further than this. I guess the rest of the document is still in a state
of flux anyway.]}
