...
designed to be simple to formalize for use with proof assistants
and automated theorem provers.

Plutus Core has a strong formal basis, being based on system F$_{\omega}$ with iso-recursive types.
There are no constructs for data types, which are represented using Scott encodings. Primitive types, such as
...
compile time. In contrast, IELE uses unbounded integers \cite{IELE?}, which never
overflow, but require that the gas used is calculated at run time. We
use F$_{\omega}$ (as opposed to system F) because it supports types indexed by
size and parameterised types (such as ``List of A'').

Notably, Plutus Core lacks explicit support for general recursion. The presence of fixed points at the type level allows us to construct term level fixed points regardless, so we omit the unnecessary term level syntax.
While this does make writing recursive programs more complicated, we intend for Plutus TX to allow easier access to term level recursion.
...
Then, in Figure \ref{fig:Plutus_core_kind_synthesis}, we define what it means for a type synthesize a kind. In Figure \ref{fig:Plutus_core_type_synthesis}, we define the type synthesis judgment, which explains how a term synthesizes a type.

In Figure \ref{fig:Plutus_core_builtins}, type synthesis rules for constants ($\con{bn}$ and $\conT{bt}$) are given in tabular form along with their reduction semantics. This table also specifies what conditions trigger an error.
...
In figure \ref{fig:Plutus_core_reduction}, we define a standard eager, small-step contextual semantics for Plutus Core in terms of the reduction relation for types (\(\typeStep{A}{A'}\)) and terms (\(\step{M}{M'}\)), which incorporates both $\beta$ reduction and contextual congruence. We denote the transitive closure of these stepping relations using the Kleene star.
