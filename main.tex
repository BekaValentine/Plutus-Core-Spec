...
\newcommand{\coeqJ}[3]{#1 ~ \vdash ~ #2 \sim #3}
...
\title{Formal Specification of\\the Plutus Napkin Language}
...
Plutus Core is a typed, eagerly evaluated $\lambda$-calculus designed for use as a transaction validation language in blockchain systems. More precisely, it is a higher-kinded variant of System F with equirecursive types. Plutus Core is intended to be both bearable to write programs with and straightforward to formalize with a proof assistant or theorem prover.

Plutus Core includes support for \emph{sized types} (and \emph{size kinds}) to facilitate reasoning about memory usage and running time of programs. This is important in blockchain settings where programs consume \emph{gas} during execution, with the amount of gas cosumed depending on the amount of space and time required. Additionally, there is no explicit support for algebraic datatypes in the language, as we intend to represent such types with their Scott encoding \cite{koopman-plasmeijer-jansen}.

The syntax of the language is presented in section 2, and section 3 describes a bidirectional typechecking algorithm for the language \cite{pfenning-bidi}\cite{christiansen-bidi}. Section 4 describes the operational semantics of the language in two ways: A sequent calculus of $\beta$-reduction, and rules for a CK machine \cite{felleisen-friedman}. The former is perhaps more suited to formal reasoning about Plutus Core, with the latter more suited to efficient implementations. Section 5 describes the prmitive terms and types of the language, and finally section 6 contains a number of examples.
...
The existence of equirecursive fixedpoint types necessities special attention be paid to equality of types, however. In figure \ref{fig:Plutus_core_coinductive_equality}, we define the judgment \(A = A'\) explicitly as a coinductive equality as in \cite{gapeyev-levin-pierce}\cite{stone-schoonmaker}. The intuition behind this notion of equality is that we perform the usual parallel structural recursion on types to compute syntactic equality, but in doing so, track which equational problems we've encountered already as super-problems to any given sub-problem, and if we re-encounter a problem, we consider the equality to hold. The syntactic decomposition is not performed in precisely one case: the fixedpoint type, where we instead decompose the fixedpoint into its one-step unfolding. Thought of in terms of a proof tree, coinductive proofs add a new mechanism of proof: where normally one can use inference rules to prove a goal inductively, in a coinductive tree you can also use reference to another node of the same syntactic form in a super-problem. Alternatively: coinductive proofs of a problem succeed when the problem repeats itself in the course of being proved.
...
In figure \ref{fig:Plutus_core_reduction}, we define a standard eager, small-step contextual semantics for Plutus Core in terms of the local ($\beta$) reduction relation for types (\(\typeReduces{A}{A'}\)) and terms (\(\reduces{M}{M'}\)), which lifts to stepping terms in arbitrary contexts (\(\typeStep{A}{A'}\) and \(\step{M}{M'}\), respectively). We make use of the transitive closure of these stepping relations via the usual Kleene star notation.
...
\bibitem{gapeyev-levin-pierce}
Gapeyev, Levin, and Pierce. \emph{Recursive Subtyping Revealed}. Journal of Functional Programming, Volume 12, Issue 6. November 2002.

\bibitem{koopman-plasmeijer-jansen}
Koopman, Plasmeijer, and Jansen. \emph{Church Encoding of Data Types Considered Harmful for Implementations}. Proceedings of the International Symposium on Implementation and Application of Functional Languages. 2014.

\bibitem{felleisen-friedman}
Felleisen and Friedman. \emph{Control Operators, the SECD Machine, and the $\lambda$-calculus}. Proceedings of the Conference on Formal Description of Programming Concepts Part III. 1986.

\bibitem{stone-schoonmaker}
Stone and Schoonmaker. \emph{Equational Theories With Recursive Types}. 2005.
